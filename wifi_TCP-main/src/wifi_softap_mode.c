...
static void configure_dhcp_cam_server(void)
{
	struct net_if *iface;
	...
	if (ret == -EALREADY) {
		LOG_ERR("DHCPv4 cam_server already running on interface");
	} else if (ret < 0) {
		LOG_ERR("DHCPv4 cam_server failed to start and returned %d error", ret);
	} else {
		LOG_INF("DHCPv4 cam_server started and pool address starts from %s",
