...
class SocketClient(QObject):
    ...
    def __init__(self, cam_address='192.168.1.1', cam_port=60010):
        ...
        self._buf = bytearray()
        self._max_frame = 10_000_000  # 10MB 防炸

    def run(self):
        try:
            self.sock.connect((self.cam_address, self.cam_port))
            print(f"[TCP] Connected to {self.cam_address}:{self.cam_port}")
...
class WifiCamHostGUI(QMainWindow):
    ...
    def _try_print_total_rate(self):
        now = time.monotonic()
        dt = now - self._sum_t0
        if dt < 1.0:
            return

        kbps = (self._sum_bytes * 8) / dt / 1000.0
        fps = self._sum_frames / dt

        self._log(f"[TOTAL] {kbps:7.2f} kbps | {fps:5.2f} fps")
    ...
    def _on_ble_frame(self, jpeg_bytes: bytes):
        # ===== 总统计（BLE）=====
        self._sum_bytes += len(jpeg_bytes)
        self._sum_frames += 1
        self._try_print_total_rate()
