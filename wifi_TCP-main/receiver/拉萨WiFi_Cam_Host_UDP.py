...
class CommandsToCamera:
    ...
    COMMANDS = {
        ...
        0x01: {"name": "SET_PICTURE_RESOLUTION", "description": "Set picture resolution",
               "parameters": {"format": None, "resolution": None}},
        0x02: {"name": "SET_VIDEO_RESOLUTION", "description": "Set video resolution",
        ...
        0x0A: {"name": "SET_EXPOSURE_GAIN_ENABLE", "description": "Set exposure gain enable",
               "parameters": {"exposure_control": None}},
        0x0C: {"name": "SET_WHITE_BALANCE_ENABLE", "description": "Set white balance enable",
               "parameters": {"white_balance_control": None}},
        0x0D: {"name": "SET_MANUAL_GAIN", "description": "Set manual gain", "parameters": {"manual_gain": None}},
        0x0E: {"name": "SET_MANUAL_EXPOSURE", "description": "Set manual exposure",
               "parameters": {"manual_exposure": None}},
        ...
        0x12: {"name": "DEBUG_WRITE_REGISTER", "description": "Debug write register",
               "parameters": {"register": None, "value": None}},
    ...
    def command_set_picture_resolution(format_code, resolution):
        ...
        command_bytes = bytes(
            [CommandsToCamera.START_CHARACTER, command_id, parameter_byte, CommandsToCamera.STOP_CHARACTER])
        command_str = " ".join(format(byte, "02X") for byte in command_bytes)
        return command_str
    ...
    def command_start_streaming_mode(resolution):
        command_id = 0x02
        command_bytes = bytes(
            [CommandsToCamera.START_CHARACTER, command_id, resolution, CommandsToCamera.STOP_CHARACTER])
        command_str = " ".join(format(byte, "02X") for byte in command_bytes)
        return command_str
    ...
    def command_set_brightness(brightness):
        ...
        command_bytes = bytes(
            [CommandsToCamera.START_CHARACTER, command_id, brightness, CommandsToCamera.STOP_CHARACTER])
        command_str = " ".join(format(byte, "02X") for byte in command_bytes)
        return command_str
    ...
    def command_set_saturation(saturation):
        ...
        command_bytes = bytes(
            [CommandsToCamera.START_CHARACTER, command_id, saturation, CommandsToCamera.STOP_CHARACTER])
        command_str = " ".join(format(byte, "02X") for byte in command_bytes)
        return command_str
    ...
    def command_set_white_balance(white_balance):
        ...
        command_bytes = bytes(
            [CommandsToCamera.START_CHARACTER, command_id, white_balance, CommandsToCamera.STOP_CHARACTER])
        command_str = " ".join(format(byte, "02X") for byte in command_bytes)
        return command_str
...
class ArducamMegaCameraDataProcess:
    ...
    def process_capture_command(self, bytesframe, frame):
        ...
        try:
            ...
            receive_dir = desktop / "receive"
            receive_dir.mkdir(parents=True, exist_ok=True)  # 如果没有文件夹就创建

            # 用时间戳命名，避免覆盖
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
            save_path = receive_dir / f"frame_{timestamp}.jpg"
...
class SocketClient(QObject):
    ...
    def send_command(self, command_str):
        try:
            ...
            # 已连接的 UDP socket：用 send()
            self.pc_socket.send(command_bytes)
        except Exception as e:
            QMessageBox.critical(None, "Error", f"Fail to send command\n{e}")
    ...
    def run(self):
        try:
            ...
            def find_start(b):
                # find b'\xFF\xAA' start; return index or -1
                return b.find(b'\xFF\xAA')
            ...
            while self.running:
                ...
                while True:
                    si = find_start(buf)
                    if si < 0:
                        # no start marker yet; keep buffer small
                        if len(buf) > 8192:
                            buf.clear()
                        break
                    ...
                    # need at least header: FF AA <type> <len:4>
                    if len(buf) < 7:
                        break

                    cmd_type = buf[2]
                    # for types with a 4-byte payload len next (0x01 picture, 0x02 info, and likely 0x03/0x05/0x06 too)
                    has_len = cmd_type in (0x01, 0x02, 0x03, 0x05, 0x06, 0x07, 0x08, 0x09)
                    if has_len:
                        payload_len = int.from_bytes(buf[3:7], 'little')
                        total_needed = 2 + 1 + 4 + payload_len + 2  # start(2)+type(1)+len(4)+payload+end(2)
                        if len(buf) < total_needed:
                            # wait for more
                            break
                        # verify end marker
                        if buf[total_needed - 2:total_needed] != b'\xFF\xBB':
                            # desync; drop the first byte and try again
                            del buf[0:1]
                            continue
                        # we have a full frame
                        frame = bytes(buf[:total_needed])
                        del buf[:total_needed]
                        self.command_receiving_signal.emit(frame)
                    else:
                        # If any cmd without explicit length (unlikely in this protocol)
                        # fall back to searching for end marker
                        end_idx = buf.find(b'\xFF\xBB', 7)
                        if end_idx < 0:
                            break
                        frame = bytes(buf[:end_idx + 2])
                        del buf[:end_idx + 2]
                        self.command_receiving_signal.emit(frame)

        except Exception as e:
            if self.running:
                QMessageBox.critical(None, "Error", f"Fail to receive packets!\n{e}")
    ...
    #
    #                 # 解析本帧大小
    #                 bytesframe = int.from_bytes(data[3:7], byteorder='little')
    #                 total_bytes_received = 0
    #
    #                 # 打印帧信息
    #                 # print(f"New Frame: FrameSize = {bytesframe} Bytes")
    #
    #                 # 把这一包的数据也累加到 buffer（如果你需要）
    ...
    #
    #                 # 实时打印接收进度
    #                 pct = (total_bytes_received / bytesframe) * 100 if bytesframe else 0
    #                 # print(f"Bytes received/FrameSize: {total_bytes_received}/{bytesframe}, {pct:.2f}%")
    #
    #                 # 检查是否完整
    ...
    #
    #                     # 在这不需要再算 corrupted 了，直接重置
    #                     # print(">>> Frame complete, handing off to processor.\n")
    #                     self.command_buffer = b''
...
class WifiCamHostGUI(QMainWindow):
    ...
    def handle_connect_button(self):
        if self.connect_button.text() == "Connect":
            self.clear_log()
            self.stream_button.setText("Start Stream")
            self.log_content_text.append(
                f"Connecting to WiFi Camera at {self.target_server[0]}:{self.target_server[1]} ...")
            ...
            self.capture_image_button.setEnabled(False)
            self.log_content_text.append(
                f"Disconnected from WiFi Camera at {self.target_server[0]}:{self.target_server[1]}")
